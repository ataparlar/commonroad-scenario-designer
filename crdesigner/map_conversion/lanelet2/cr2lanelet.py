...
class CR2LaneletConverter:
        ...
        for idx, ll in enumerate(self.lanelet_network.lanelets):
            ...
            if len(ll.traffic_lights) > 0:
                ...
                self.osm.add_regulatory_element(
                    RegulatoryElement(regulatory_element_id, traffic_light_reference_list, ref_line=way_list,
    ...
    def _convert_traffic_light(self, light: TrafficLight):
        ...
        lat2, lon2 = self.transformer.transform(self.origin_utm[0] + light.position[0] + 0.025,
                                                self.origin_utm[1] + light.position[1] + 0.025)
        if(not autoware):
            lat3, lon3 = self.transformer.transform(self.origin_utm[0] + light.position[0] - 0.025,
                                                self.origin_utm[1] + light.position[1] - 0.025)
            id3 = self.id_count
        ...
        if(self._config.use_local_coordinates):
            ...
            localx2, localy2 = light.position[0] - 0.025, light.position[1] + 0.025
            localx3, localy3 = light.position[0] - 0.025, light.position[1] - 0.025
        ...
        if(autoware):
            self.osm.add_way(Way(traffic_light_id, [id1, id2],
